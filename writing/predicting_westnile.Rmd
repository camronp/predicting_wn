...
#try to remove double events - does not work - also tried unique()
all_storm %>%
  group_by(year, week, event_type) %>%
  distinct()
...
all_soil_temps$avg_soiltemp_4in_sod <- as.numeric(all_soil_temps$avg_soiltemp_4in_sod)

all_soil_temps %>%
  na.omit() %>%
  group_by(year, week) 
  #mutate(avg_sod = mean(avg_soiltemp_4in_sod))
  summarise_at(vars(avg_soiltemp_4in_sod), mean)
  
#had to change class to numeric :)  
...
#worked for some of the precipitation weeks
weather_data %>%
  group_by(year, week) %>%
  mutate(avg_precip = mean(precip))

#works for all?
weather_data %>% 
  group_by(year, week) %>% 
  summarise_at(vars(precip), mean)


```

Attempt to join all of the data points

```{r}

#all_data <- full_join(mosquito_data, all_storm, all_soil_temps, weather_data)

all_data <- full_join(mosquito_data, all_storm, by = NULL)
all_data <- full_join(all_data, all_soil_temps, by = NULL)
all_data <- full_join(all_data, weather_data, by = NULL)
```
