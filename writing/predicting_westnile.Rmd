...
equation_test <- formula(prop_pos_mos ~ fp(week) +fp(avg_temp) + fp(precip_mean) + 
                           fp(avg_soil_t_4) + fp(avg_soil_t_2) )
...
equation_test <- formula(prop_pos_mos ~ fp(precip_mean) + fp(avg_soil_t_4) + fp(avg_soil_t_2) 
                         + fp(week) +fp(avg_temp))

# build regression tree model
tree_fit1 <- rpart(prop_pos_mos ~ precip_mean + avg_soil_t_4 + avg_soil_t_2 + week + avg_temp,
...
forest_pre <- predict(forest_fit, data_frame(precip_mean = 0.078, avg_soil_t_4 = 72.1, avg_soil_t_2 = 200, week = 34, avg_temp = 75), type = "class")
...
ggplot(all_data, aes(x = avg_temp, 
  ...
  labs(x = "Mean Temperature (°F)", y = "Mosquito Count",
       color = "Number of Mosquitoes Positive for WNV")
...
ggplot(all_data, aes(x = precip_mean, 
  ...
  labs(x = "Mean Precipitation (inches)", y = "Mosquito Count",
       color = "Mosquitoes Positive for WNV") 
