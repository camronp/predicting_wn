...
equation_test <- formula(prop_pos_mos ~ fp(precip_mean) + fp(avg_soil_t_4) + fp(avg_soil_t_2) + fp(week) +fp(avg_temp))
...
predict.lm(mos_mod, data_frame(precip_mean = 0.078, avg_soil_t_4 = 72.1, avg_soil_t_2 = 74.7, week = 34, avg_temp = 75))

#week 34 2017:  72.1 t4, 74.7 t2, 0.078 precip --> actual propotion = 0.52, 
```
First attempt at regression tree - could also try random forest
*Pruning regressoin tree
```{r}
...
plot(tree_fit, uniform=TRUE, 
  	main="Classification Tree for All data")
text(tree_fit, cex=.8)

post(tree_fit, file = "./data/try1.ps", title = "try1")
...
importance(forest_fit) # importance of each predictor
^change this into dataframe, change first column to column

class(varImpPlot(forest_fit,main="Important variables Random Forest"))


ggplot(forest_fit, aes(x=reorder(w,v), y=v,fill=v))+ 
...
#wind speed
ggplot(all_data, aes(x = max_wind, y = total_no_mos, color = pos_mos_no)) +
  geom_point()
