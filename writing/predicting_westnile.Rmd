...
data_wo_char <- all_data %>%
  select(-event_type, -year, -TMIN_avg, -TMAX_avg) %>%
...
Plot proportion of positive mosquitoes
...
ggplot(new_mos, aes(x = week, y = no_mos, fill = result)) +
  geom_bar(stat = "identity") +
  labs(x = "Week of year", y = "Number of collected mosquitoes") +
  ggtitle("Positive West Nile Virus Mosquitoes 2016 - 2018")


```

Leaflet map of where mosquitoes are found and the proportion of WNV mosquitoes
...
popup_info <- paste0("<b>Proportion WNV mosquitoes<b>", round(final_mosquito_leaf$prop_pos_mos, digits = 2))
...
leaflet(data = final_mosquito_leaf) %>%
  ...
  addLegend(pal = pal, values = ~prop_pos_mos, opacity = 0.9, title = "proportion \n WNV mosquitoes") 
...
## Multivariate fractional polynomial and linear model

Build multivariate fractional polynomial prediction model for proportion of positive mosquitos

```{r}

# input the variables we believe to be predictive
equation_test <- formula(prop_pos_mos ~ fp(week) +fp(avg_temp) + fp(precip_mean) + 
                           fp(avg_soil_t_4) + fp(avg_soil_t_2) + fp(WDMV))
...
predict.lm(linear_fit, data_frame(precip_mean = 0.078, avg_soil_t_4 = 72.1, avg_soil_t_2 = 200, week = 34, avg_temp = 75))
...
tree_fit1 <- rpart(prop_pos_mos ~ precip_mean + avg_soil_t_4 + avg_soil_t_2 + week + avg_temp + WDMV,
  	method="class", data = train_all_data, cp = 0.01)
...
# plot decision tree 
prp(tree_fit, main="Classification Tree for Predicting Proportion of Positive Mosquitoes", fallen.leaves = TRUE)
...
forest_fit <- randomForest(prop_pos_mos ~ precip_mean + avg_soil_t_4 + avg_soil_t_2 + week + avg_temp + WDMV,
...
ggplot(impor_var, aes(x = reorder(variables, -MSE), y = MSE, fill = variables)) +
  geom_bar(stat = "identity") +
  ylab("Mean square error (%)")+
  xlab("Variables") +
  ggtitle("Importance of Random Forest Variables")

forest_pre <- predict(forest_fit, data_frame(precip_mean = 0.078, avg_soil_t_4 = 72.1, avg_soil_t_2 = 200, week = 34, avg_temp = 75, WDMV = 5), type = "class")
...
# build regression tree model
tree_fit_all <- rpart(prop_pos_mos ~ precip_mean + avg_soil_t_4 + avg_soil_t_2 + week + avg_temp + WDMV,
...
# build random forest prediction model
forest_fit_all <- randomForest(prop_pos_mos ~ precip_mean + avg_soil_t_4 + avg_soil_t_2 + week + avg_temp + WDMV,
...
```
```{r parameters for tiff device}
# tiff("../folder/name.tiff", height = 750, width = 1000)
# dev.off()
```
