...
When we were researching things that may influence WNV infection of mosquitoes, we found a paper that explained soil temperature, precipitation, wind speed, and relative humidity have some sort of association with WNV infected mosquitoes in Northern Greece. With that in mind, we chose NOAA weather data. We had quite a few columns which had mostly NAs so we wanted to filter it down to just precipitation for this particular project. We grouped the data by year and week and found the mean precipitation for that week and year. 
...
ggplot(all_data, aes(x = avg_soil_t_4, 
                     y = total_no_mos, color = pos_mos_no)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +
  geom_smooth(se = FALSE, color = "black") +
...
# avg soil temp 2 inch
ggplot(all_data, aes(x = avg_soil_t_2, 
                     y = total_no_mos, color = pos_mos_no)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +
  geom_smooth(se = FALSE, color = "black") +
  labs(x = "Mean Soil Temperature, 2 inch (°F)", y = "Mosquito Count",
       color = "Mosquitoes \nPositive for WNV") 

# wind
ggplot(all_data, aes(x = WDMV, 
                     y = total_no_mos, color = pos_mos_no)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +
  geom_smooth(se = FALSE, color = "black") +
  labs(x = "Max 24-h Wind Movement (miles)", y = "Mosquito Count",
       color = "Mosquitoes \nPositive for WNV")

# mean max temp
ggplot(all_data, aes(x = TMAX_avg, 
                     y = total_no_mos, color = pos_mos_no)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +
  geom_smooth(se = FALSE, color = "black") +
  labs(x = "Mean Maximum Temperature (°F)", y = "Mosquito Count",
       color = "Mosquitoes \nPositive for WNV")


# average min temp
ggplot(all_data, aes(x = TMIN_avg, 
                     y = total_no_mos, color = pos_mos_no)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +
  geom_smooth(se = FALSE, color = "black") +
  labs(x = "Mean Minimum Temperature (°F)", y = "Mosquito Count",
       color = "Mosquitoes \nPositive for WNV")


# precip mean
ggplot(all_data, aes(x = precip_mean, 
                     y = total_no_mos, color = pos_mos_no)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +
  geom_smooth(se = FALSE, color = "black") +
...
To build a prediction model for proportion of positive mosquitoes, we tested three type of models: a linear prediction model, a regression tree, and random forest. For the models, we split the data into 2 sections: the first would contains the data collected from 2016 and 2017 to train the data sets, and the second contains the 2018 data to test the accuracy of the models.
...
There was a large number of positive WNV tested mosquitoes the 30th week of 2016. The dates for this time were July 25th through July 31st. The precipitation mean during that time was 0.467 inches. There is a negative association between the average soil temperatures (in both 2 inch bare soil and in 4 inch sod) and precipitation.   

WNV incidence goes up if the daily high temperature in the spring and early summer is warmer than 81 degrees more often than normal (ScienceNetLinks, 2018). This is because **Culex pipiens** will peak earlier and thus more WNV cases. 
...
* Use separate "testing" notation for markdown chunks
